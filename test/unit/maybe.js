...
describe('Maybe', () => {
  it('should Nothing', () => {
    expect(Nothing).to.be.instanceOf(Maybe)
    expect(Nothing).not.to.be.instanceOf(Just)
  })

  it('should just', () => {
    let j = just(1)
    expect(j).to.be.instanceOf(Maybe)
    expect(j).to.be.instanceOf(Just)
  })

  it('should fmap Nothing', () => {
    let m = Nothing
    expect(m.map(v => v.toString())).to.equal(Nothing)
  })
  ...
  it('should <*>', () => {
    ...
    expect(m2).to.be.instanceOf(Just)

    let m3 = (f) ['<*>'] (m1)
    expect(m3.value).to.equal(2)

    let m4 = (Nothing) ['<*>'] (m1)
    expect(m4).to.equal(Nothing)
  })
  ...
  it('should *>', () => {
    ...
    let m2 = just(2)
    let m3 = (m1) ['*>'] (m2)
    expect(m3).to.be.instanceOf(Just)
    expect(m3.value).to.equal(2)
  })
